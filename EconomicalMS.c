...
int main()
{
	...
	long double len, cur_min_len=HUGE_VALL;
	...
	cur_min_ms=(int*)malloc((X*X)*sizeof(int));
	if(cur_min_ms==NULL){
		fprintf(stderr, "error: failed to malloc cur_min_ms\n");
		exit(EXIT_FAILURE);
	}
	...
	printf("The most economical MS: ");
	for(i=0; i<X*X; i++)
		printf("%d%c", cur_min_ms[i], i!=X*X-1?' ':'\n');
	printf("The length: %Lf\n", cur_min_len);
